...
# === Scanner Process ===
# bluepy is the only scanner that coexists with bluetoothd, which gatttool needs
# for recording (raw hcitool/HCI scanning requires exclusive control of the
# adapter and breaks recording). Each scanner.scan() re-discovers every in-range
# camera and keeps last_seen fresh. A SHORT window makes detection more frequent
# than the original scan(4); lower SCAN_SECONDS = faster but slightly more CPU.
SCAN_SECONDS = 2
